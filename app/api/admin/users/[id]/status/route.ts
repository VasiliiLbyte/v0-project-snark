...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const parsed = adminPortalUserStatusSchema.safeParse(body)
    if (!parsed.success) {
      const payload = apiErrorSchema.parse({
        error: "Укажите флаг активности",
        code: "INVALID_PAYLOAD",
      })
      return NextResponse.json(payload, { status: 400 })
    }

    if (parsed.data.isActive === false && parsedId.data === auth.userId) {
      const payload = apiErrorSchema.parse({
    ...
    try {
      const item = await getPortalRepositoryServer().updateAdminPortalUserStatus(
        parsedId.data,

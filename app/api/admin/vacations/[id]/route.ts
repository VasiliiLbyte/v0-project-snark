...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const parsed = vacationAdminUpdateSchema.safeParse(body)
    if (!parsed.success) {
      const payload = apiErrorSchema.parse({
    ...
    const updated = await getPortalRepositoryServer().updateAdminVacation(parsedId.data, {
      ...parsed.data,
    ...
    await writeAuditLog({
      userId: auth.userId,
      action: "admin:vacations:update",
      resourceType: "vacations",
      resourceId: parsedId.data,
      statusCode: 200,
    })

    return NextResponse.json(vacationItemSchema.parse(updated))
  } catch (error) {
    ...
    const payload =
      status === 500
        ? apiErrorSchema.parse({
            error: "Не удалось обновить заявку на отпуск",
            code: "INTERNAL_ERROR",
          })
        : apiErrorSchema.parse({

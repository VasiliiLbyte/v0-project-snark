...
import { addChannelMembers } from "@/lib/repositories/chat.repository"
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const status = known.status ?? (message.includes("доступ") ? 403 : message.includes("найден") ? 404 : 500)
    return NextResponse.json(
      apiErrorSchema.parse({
        error: status === 500 ? message : (known.message ?? message),
        code: status === 403 ? "FORBIDDEN" : status === 404 ? "NOT_FOUND" : status === 500 ? "INTERNAL_ERROR" : (known.code ?? "AUTH_ERROR"),
      }),
      { status }
    )
  }
